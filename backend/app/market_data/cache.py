...
Single writer (the active provider), many readers (SSE connections). See
planning/MARKET_DATA_DESIGN.md section 4.
"""
...
class PriceCache:
    ...
    No locking: FastAPI's asyncio event loop is single-threaded and every
    method here is synchronous (no `await` between the read and the write),
    so there's no interleaving to guard against.
    """
